...
/** Set Imgix parameters on an Unsplash delivery URL, preserving its own
 *  (`ixid` is attribution tracking and must survive). */
...
async function blurDataUrl(url) {
  try {
    ...
    const res = await fetch(
      sized(url, { w: BLUR_WIDTH, q: 35, fm: "jpg", fit: "max", cs: "tinysrgb" }),
      {
        cache: "force-cache",
...
function normalize(summary, detail, blur) {
  ...
  return {
    ...
    full: sized(summary.urls.full ?? summary.urls.raw, {
      w: LIGHTBOX_WIDTH,
      q: 80,
      fit: "max",
      auto: "format",
